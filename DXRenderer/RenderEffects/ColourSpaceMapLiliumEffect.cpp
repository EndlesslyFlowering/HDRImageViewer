...
#include "pch.h"
#include <initguid.h>
#include "ColourSpaceMapLiliumEffect.h"
#include "..\Common\BasicReaderWriter.h"

#define XML(X) TEXT(#X)

ColourSpaceMapLiliumEffect::ColourSpaceMapLiliumEffect() :
    m_refCount(1),
    m_constants{}
{
}

HRESULT __stdcall ColourSpaceMapLiliumEffect::CreateColourSpaceMapLiliumImpl(_Outptr_ IUnknown** ppEffectImpl)
{
    // Since the object's refcount is initialized to 1, we don't need to AddRef here.
    *ppEffectImpl = static_cast<ID2D1EffectImpl*>(new (std::nothrow) ColourSpaceMapLiliumEffect());
...
HRESULT ColourSpaceMapLiliumEffect::Register(_In_ ID2D1Factory1* pFactory)
{
    ...
    PCWSTR pszXml =
        XML(
            ...
            <Effect>
                <!-- System Properties -->
                <Property name='DisplayName' type='string' value='CreateColourSpaceMapLilium'/>
    ...
    return pFactory->RegisterEffectFromString(
        CLSID_CustomColourSpaceMapLiliumEffect,
        pszXml,
        nullptr, // No custom properties
        0,       // No custom properties
        CreateColourSpaceMapLiliumImpl
        );
}

IFACEMETHODIMP ColourSpaceMapLiliumEffect::Initialize(
    ...
    )
{
    ...
    try
    {
        ...
        data = reader->ReadData("DXRenderer\\ColourSpaceMapLiliumEffect.cso");
    }
    ...
    HRESULT hr = pEffectContext->LoadPixelShader(GUID_ColourSpaceMapLiliumPixelShader, data->Data, data->Length);
...
HRESULT ColourSpaceMapLiliumEffect::UpdateConstants()
{
...
IFACEMETHODIMP ColourSpaceMapLiliumEffect::PrepareForRender(D2D1_CHANGE_TYPE changeType)
{
...
IFACEMETHODIMP ColourSpaceMapLiliumEffect::SetGraph(_In_ ID2D1TransformGraph* pGraph)
{
    return E_NOTIMPL;
}
...
IFACEMETHODIMP ColourSpaceMapLiliumEffect::SetDrawInfo(_In_ ID2D1DrawInfo* pDrawInfo)
{
    m_drawInfo = pDrawInfo;

    return m_drawInfo->SetPixelShader(GUID_ColourSpaceMapLiliumPixelShader);
}

// Calculates the mapping between the output and input rects.
IFACEMETHODIMP ColourSpaceMapLiliumEffect::MapOutputRectToInputRects(
...
IFACEMETHODIMP ColourSpaceMapLiliumEffect::MapInputRectsToOutputRect(
...
IFACEMETHODIMP ColourSpaceMapLiliumEffect::MapInvalidRect(
    UINT32 inputIndex,
...
IFACEMETHODIMP_(UINT32) ColourSpaceMapLiliumEffect::GetInputCount() const
{
    return 1;
}
...
IFACEMETHODIMP_(ULONG) ColourSpaceMapLiliumEffect::AddRef()
{
    m_refCount++;
    return m_refCount;
}

IFACEMETHODIMP_(ULONG) ColourSpaceMapLiliumEffect::Release()
{
    m_refCount--;
...
IFACEMETHODIMP ColourSpaceMapLiliumEffect::QueryInterface(
    _In_ REFIID riid,
