...
#include "pch.h"
#include <initguid.h>
#include "LuminanceHeatmapLiliumEffect.h"
#include "..\Common\BasicReaderWriter.h"

#define XML(X) TEXT(#X)

LuminanceHeatmapLiliumEffect::LuminanceHeatmapLiliumEffect() :
    m_refCount(1),
    m_constants{}
{
}

HRESULT __stdcall LuminanceHeatmapLiliumEffect::CreateLuminanceHeatmapLiliumImpl(_Outptr_ IUnknown** ppEffectImpl)
{
    // Since the object's refcount is initialized to 1, we don't need to AddRef here.
    *ppEffectImpl = static_cast<ID2D1EffectImpl*>(new (std::nothrow) LuminanceHeatmapLiliumEffect());
...
HRESULT LuminanceHeatmapLiliumEffect::Register(_In_ ID2D1Factory1* pFactory)
{
    ...
    PCWSTR pszXml =
        XML(
            ...
            <Effect>
                <!-- System Properties -->
                <Property name='DisplayName' type='string' value='LuminanceHeatmapLilium'/>
    ...
    return pFactory->RegisterEffectFromString(
        CLSID_CustomLuminanceHeatmapLiliumEffect,
        pszXml,
        nullptr, // No custom properties
        0,       // No custom properties
        CreateLuminanceHeatmapLiliumImpl
        );
}

IFACEMETHODIMP LuminanceHeatmapLiliumEffect::Initialize(
    ...
    )
{
    ...
    try
    {
        ...
        data = reader->ReadData("DXRenderer\\LuminanceHeatmapLiliumEffect.cso");
    }
    ...
    HRESULT hr = pEffectContext->LoadPixelShader(GUID_LuminanceHeatmapLiliumPixelShader, data->Data, data->Length);
...
HRESULT LuminanceHeatmapLiliumEffect::UpdateConstants()
{
...
IFACEMETHODIMP LuminanceHeatmapLiliumEffect::PrepareForRender(D2D1_CHANGE_TYPE changeType)
{
...
IFACEMETHODIMP LuminanceHeatmapLiliumEffect::SetGraph(_In_ ID2D1TransformGraph* pGraph)
{
    return E_NOTIMPL;
}
...
IFACEMETHODIMP LuminanceHeatmapLiliumEffect::SetDrawInfo(_In_ ID2D1DrawInfo* pDrawInfo)
{
    m_drawInfo = pDrawInfo;

    return m_drawInfo->SetPixelShader(GUID_LuminanceHeatmapLiliumPixelShader);
}

// Calculates the mapping between the output and input rects.
IFACEMETHODIMP LuminanceHeatmapLiliumEffect::MapOutputRectToInputRects(
...
IFACEMETHODIMP LuminanceHeatmapLiliumEffect::MapInputRectsToOutputRect(
...
IFACEMETHODIMP LuminanceHeatmapLiliumEffect::MapInvalidRect(
    UINT32 inputIndex,
...
IFACEMETHODIMP_(UINT32) LuminanceHeatmapLiliumEffect::GetInputCount() const
{
    return 1;
}
...
IFACEMETHODIMP_(ULONG) LuminanceHeatmapLiliumEffect::AddRef()
{
    m_refCount++;
    return m_refCount;
}

IFACEMETHODIMP_(ULONG) LuminanceHeatmapLiliumEffect::Release()
{
    m_refCount--;
...
IFACEMETHODIMP LuminanceHeatmapLiliumEffect::QueryInterface(
    _In_ REFIID riid,
